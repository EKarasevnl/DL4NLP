...
#SBATCH --gpus=1
#SBATCH --job-name=multilang_nld
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=16
#SBATCH --time=08:00:00
#SBATCH --output=outputs/multilang_eval/multilang_nld_Latn_%A.out
#SBATCH --mem=128G
...
echo "Starting multilingual evaluation for source language: nld_Latn"
echo "Languages: $LANGS"
echo "FLORES path: $FLORES_DIR"
echo "WMT24++ path: $WMT24PP_DIR"

python multilang_eval.py   --flores_path "$FLORES_DIR"   --wmt24pp_path "$WMT24PP_DIR"   --langs "$LANGS"   --src_lang nld_Latn   --mode int4   --split devtest   --batch_size 8   --max_new_tokens 256   --output_dir outputs/multilang_eval

echo "Done. Scores written to outputs/multilang_eval/multilang_scores_nld_Latn.csv"
