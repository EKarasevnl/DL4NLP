...
#SBATCH --gpus=1
#SBATCH --job-name=multilang_zul_baseline
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=16
#SBATCH --time=08:00:00
#SBATCH --output=outputs/multilang_eval/multilang_zul_Latn_baseline_%A.out
#SBATCH --mem=128G
...
echo "Starting multilingual evaluation for source language: zul_Latn, mode: baseline"
echo "Languages: $LANGS"
echo "FLORES path: $FLORES_DIR"
echo "WMT24++ path: $WMT24PP_DIR"

python multilang_eval.py \
  --flores_path "$FLORES_DIR" \
  --wmt24pp_path "$WMT24PP_DIR" \
  --langs "$LANGS" \
  --src_lang zul_Latn \
  --mode baseline \
  --split devtest \
...
echo "Done. Scores written to outputs/multilang_eval/multilang_scores_zul_Latn_baseline.csv"
