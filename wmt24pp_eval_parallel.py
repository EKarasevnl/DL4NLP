...
def run(cfg: EvalConfig) -> int:
    ...
    csv_path = os.path.join(cfg.output_dir, f"wmt24pp_scores_{cfg.mode}.csv")
...
def parse_args() -> EvalConfig:
    ...
    # Convert NLLB language codes to WMT24++ 2-letter codes if needed
    converted_langs = []
    nllb_to_wmt = {v: k for k, v in NLLB_CODE_BY_WMT24PP.items()}
    
    for lang in langs:
        if lang in nllb_to_wmt:
            # Convert from NLLB code to WMT code
            converted_langs.append(nllb_to_wmt[lang])
        elif lang in NLLB_CODE_BY_WMT24PP:
            # Already a WMT code
            converted_langs.append(lang)
        else:
            print(f"Warning: Unknown language code {lang}")
            converted_langs.append(lang)
